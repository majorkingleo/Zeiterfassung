...
public class ModuleLauncher extends BaseModuleLauncher implements
	...
	public ModuleLauncher(String[] args) {
		...
		if (at.redeye.Dongle.AppliactionModes.getAppliactionModes()
			...
			title = getStartupParam("apptitle", "apptitle", "APPTITLE",
					"ZES-DEV");
		} else {
		...
		FrameWorkConfigDefinitions.LookAndFeel
				.addPrmListener(new PrmListener() {
					...
					PrmDefaultCheckSuite checker = new PrmDefaultCheckSuite(
							PrmDefaultChecksInterface.PRM_IS_LOOKANDFEEL);
	...
	protected void invoke() {
		...
		System.out.println("before setCommonLoggingLevel time: "
		...
		System.out.println("before loadDBConnectionFromSetup time: "
				+ (System.currentTimeMillis() - start));
		...
		System.out.println("before autoImportDBStep2 time: "
				+ (System.currentTimeMillis() - start));
		...
		System.out.println("before check_config time: "
				+ (System.currentTimeMillis() - start));
			...
			ConfigWizard config_wizard = new ConfigWizard(root,
					new WizardListener() {
						...
						public boolean onStateChange(
								WizardStatus currentWizardStatus) {
		...
		System.out.println("PrmDBInit time: "
				+ (System.currentTimeMillis() - start));

		System.out.println("before UM time: "
				+ (System.currentTimeMillis() - start));
	...
	public void accessGranted() {
		...
		System.out.println("accessGranted time: "
				+ (System.currentTimeMillis() - start));
		...
		if (fileAppender != null) {
			try {
				...
				RollingFileAppender rfa = new RollingFileAppender(
						fileAppender.getLayout(), filename);
		...
		System.out.println("XXXXXXXXX time: "
				+ (System.currentTimeMillis() - start));

		if (startup_mon != null && startup_year != null) {
			try {
		...
		if (failed) {
			// Don't start as thread, because BaseDialog attempts wrong closing
			// of
			// application
	...
	public void loadDefaultUser(final String user) {
		AutoLogger al = new AutoLogger(BaseModuleLauncher.class.getName()) {
			...
			public void do_stuff() throws Exception {
				...
				DBPb pb = new DBPb();
				List<DBPb> res = trans.fetchTable2(pb,
						"where " + trans.markColumn(pb.login) + "='" + user
								+ "'");
				...
				if (res.size() > 0) {
					...
					logger.info("user " + user
							+ " not found in database. Creating default user.");
		...
		if (al.isFailed()) {
			...
			map.put("pwd", "  ---  ");
			map.put("login", user);
			map.put("plevel", UserManagementInterface.UM_PERMISSIONLEVEL_ADMIN);
