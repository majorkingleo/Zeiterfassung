...
public class UserData extends javax.swing.JPanel {
	...
	public UserData(Root root, BaseDialog base) {
		...
		DBPermissionLevelInteger level = new DBPermissionLevelInteger("level",
				"level");
	...
	private void initComponents() {
		...
		jCUserRights.setModel(new javax.swing.DefaultComboBoxModel(
				...
				.addGroup(
						layout.createSequentialGroup()
								...
								.addPreferredGap(
										javax.swing.LayoutStyle.ComponentPlacement.RELATED,
										21, Short.MAX_VALUE)
								.addComponent(jBCreate).addContainerGap()));
	...
	private void jBCreateActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jBCreateActionPerformed
		...
		logger.info(String.format("Neuen Benutzer Anlegen: %s %s %s level: %d",
				pb.title.toString(), pb.surname.toString(), pb.name.toString(),
				pb.plevel.getValue()));
		...
		if (jTloginname.getText().trim().isEmpty()) {
			JOptionPane
					.showMessageDialog(
							jBCreate,
							StringUtils
									.autoLineBreak("Es muß zumindest ein Benutzername angegeben werden."),
		...
		if (passwd1.isEmpty()) {
			JOptionPane.showMessageDialog(getParent(), StringUtils
					.autoLineBreak("Bitte geben Sie ein Passwort ein."),
					"Fehler", JOptionPane.ERROR_MESSAGE);
		...
		if (!passwd1.equals(passwd2)) {
			JOptionPane.showMessageDialog(getParent(), StringUtils
					.autoLineBreak("Die Passwörter unterscheiden sich!"),
					"Fehler", JOptionPane.ERROR_MESSAGE);
		...
		new AutoMBox(UserData.class.getName()) {
			...
			public void do_stuff() throws Exception {
				...
				List<DBPb> other_pers = trans.fetchTable2(new DBPb(), "where "
						+ trans.markColumn(pb.login) + " = '" + pb.login + "'");
				...
				if (!other_pers.isEmpty()) {
					...
					JOptionPane
							.showMessageDialog(
									getParent(),
									StringUtils
											.autoLineBreak("Der Benutzername ist bereits in Verwendung. Bitte wählen Sie einen anderen."),
									"Fehler", JOptionPane.ERROR_MESSAGE);
					...
					pb.id.loadFromCopy(base
							.getNewSequenceValue(AdminDlg.UM_ID_SEQ));
