...
public class TimeEntryCache {
	...
	List<DBStrukt> entries = new ArrayList<DBStrukt>();
	boolean outdated = true;
	HashMap<String, AutoCompleteInfo> info_map = new HashMap<String, AutoCompleteInfo>();
	GetUserPerMonthRecord get_upm = null;

	public DBUserPerMonth getUPM(Transaction trans, int userid, DateMidnight day)
		...
		DBUserPerMonth upm = get_upm.getValidRecordForMonthCached(trans,
				userid, day.getYear(), day.getMonthOfYear());
		return upm;
	}

	public Vector<DBStrukt> getEntries(Transaction trans, int userid,
			DateMidnight day) throws SQLException,
		...
		// System.out.println( "Requested: " + day.toString() + " from " + begin
		// + " till " + end );
		...
		for (int i = 0; i < entries.size(); i++) {
			...
			if (begin <= millis && millis < end) {
				// System.out.println( "Found: " + d.toString() );
				res.add(e);
			} else {
				// System.out.println( "not match: " + d.toString() +
				// " millis: " + millis );
			}
			/*
			 * if( end > millis ) break;
	...
	protected void refresh(Transaction trans, int userid, DateMidnight day)
			throws SQLException, TableBindingNotRegisteredException,
			UnsupportedDBDataTypeException, WrongBindFileFormatException {
		if (!outdated)
			return;
		...
		String where = "where " + trans.markColumn("user") + "=" + userid + " "
				+ " and " + trans.getPeriodStmt("from", dm_from, dm_to)
